AFRAME.registerComponent('locker-puzzle', {
  ...
  init() {
    ...
    this.solveAudio = new Audio('assets/audio/solve.mp3')
    this.unlockAudio = new Audio('assets/audio/unlock.mp3')
    ...
    this.el.addEventListener('model-loaded', () => {
      ...
      console.log('Locker puzzle initialized')
    })
  },
    ...
    do {
      ...
      for (let i = 0; i < this.knobs.length; i++) {
        ...
        this.knobs[i].targetRotation =
          THREE.MathUtils.degToRad(-randomValue * this.ROTATION_STEP)
  ...
  onPointerDown(event) {
    ...
    if (this.topSolved && this.bottomSolved) return
    ...
    const hitMeshes = this.knobs.map(k => k.hitMesh)
    ...
    const knob = hit.userData.knobData
  ...
  onPointerMove(event) {
    ...
    const stepCount =
      Math.trunc(deltaY / this.SWIPE_THRESHOLD)

    if (stepCount === this.lastProcessedStep) return
    ...
    const stepsToMove =
      Math.abs(stepCount - this.lastProcessedStep)
    ...
    this.lastProcessedStep = stepCount
  },
  ...
  tick(time, delta) {
    ...
    const lerpSpeed = Math.min(delta * 0.02, 0.25)
